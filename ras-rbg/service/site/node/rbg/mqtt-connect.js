...
var selectDeviceStatus = function(device_id, callback) {
    redis_client.send_command('hgetall', ["ui.rbg.device-status-" + device_id], function (error, result) {
        if (error) {
...
redis_client.send_command('SELECT', [gv.config.redis_db], function (error, result) {
    ...
    // TODO : invalid device_id check !!,
    // --> ex : "ui.rbg.device-status-mosqpub|406239-cccpr201" (console 을 죽인다.)
    ...
    //
    var user_data = {};
    redis_client.send_command('hgetall', ["config.rbg.devices"], function (error, result) {
        if (error) {
            console.log('-----err', param1);
            // TODO exit
        }
        ...
        async.times(user_data.devices.length, function(n, next) {
            ...
            selectDeviceStatus(did, function(err, data) {
                if (data === null) data = [];   // 2019.09.29, ui 가 없을 경우 방어코드

                if (device_id === did) {
                    data.status = con_status;
                }    // 자기 자신은 접속 최신 상태를 반영한다.
            ...
            console.log(es.makeActionToMQTT("rbg", pTopic, "connect", {
                "cmd" : "server-event",
                "mqtt_data" : mqttData
            }));
            ...
            // UI Object update
            es.setUIObject(redis_client, "rbg", "device-status-" + device_id, {
            ...
            es.publishTopicData(redis_client, "rbg", pTopic, mqttData);
